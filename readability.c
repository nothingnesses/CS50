...
Text luhn_instance(Text input, int letters, int words, int sentences, int increment) {
  return (Text) {
    input.text,
    letters,
    words,
...
Text text_evaluate(Text input) {
  ...
  ? is_letter(current_character)
    ...
    ? text_evaluate(luhn_instance(input, input.letters + 1, input.words, input.sentences, 1))
    : (is_whitespace(current_character) && !is_whitespace(previous_character))
      ? text_evaluate(luhn_instance(input, input.letters, input.words + 1, input.sentences, 1))
      : (is_end_mark(current_character) && !is_end_mark(previous_character))
        ? text_evaluate(luhn_instance(input, input.letters, input.words, input.sentences + 1, 1))
        : text_evaluate(luhn_instance(input, input.letters, input.words, input.sentences, 1))
